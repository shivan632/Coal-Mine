...
export interface PredictiveHazardMetric {
  zone: MineZoneId;
  ch4VelocityPercentPerMin: number; // Rate of change d(CH4)/dt
  coVelocityPpmPerMin: number;      // Rate of change d(CO)/dt
  tempVelocityDegPerMin: number;    // Rate of change d(Temp)/dt
  projectedCh4_10m: number;         // Projected CH4 in 10 minutes
  projectedCh4_30m: number;         // Projected CH4 in 30 minutes
  timeToBreachSeconds: number | null; // Seconds until CH4 >= 1.25% (null if stable)
...
export interface ReplayTimelinePoint {
  offsetSeconds: number; // e.g. -300 to +300
  timestamp: number;
